...
def SLCT(para, log_format, rex):
    ...
    if not os.path.isfile('../SLCT/slct'):
        try:
            ...
            subprocess.check_output('gcc -o slct -O2 cslct.c', 
...
def extract_command(para, logname):
    ...
    if parajTF:
        input = './slct -j -o ' + 'slct_outliers.log -r -s ' + str(support) + ' ' + logname
    else:
        input = './slct -o ' + 'slct_outliers.log -r -s ' + str(support) + ' ' + logname
    return input
